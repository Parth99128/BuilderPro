...
async def dashboard(db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    return DashboardOut(
        total_projects=len(projects), active_projects=len(active),
        total_budget=total_budget, total_spent=total_spent, total_remaining=total_budget - total_spent,
        pending_payments=pending, total_workers=len([w for w in workers if w.status == "active"]),
        total_materials=len(materials), low_stock_count=low_stock, overdue_invoices=overdue,
        weekly_payroll=weekly, monthly_payroll=monthly,
    )
...
async def demand_forecast(project_id: str = None, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    q = select(Material)
    if project_id:
        q = q.where(Material.project_id == project_id)
...
async def run_reconciliation(db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    # Mark reconciled invoices in DB
    for inv in invoices:
